...
Rails.application.configure do
  ...
  if Rails.root.join("tmp/minio-dev.txt").exist?
    ...
    config.x.content_security_policy.connect_src = "http://minio.jetkb.localhost:39000"
    config.x.content_security_policy.img_src = "http://minio.jetkb.localhost:39000"
  else
  ...
  config.hosts = [
    "app.jetkb.localhost",
    "jetkb.localhost", # here for backwards compat, but will have CORS errors when using minio
    "localhost",
    "127.0.0.1",
    /jetkb-\d+/,   # review apps: jetkb-123, jetkb-456:3000
