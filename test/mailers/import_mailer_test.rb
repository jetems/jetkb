...
class ImportMailerTest < ActionMailer::TestCase
  test "completed" do
    ...
    assert_equal "Your #{JetKB::Brand::APP_NAME} account import is done", email.subject
  ...
  test "failed with no reason" do
    ...
    assert_equal [ "david@37signals.com" ], email.to
    assert_equal "Your #{JetKB::Brand::APP_NAME} account import failed", email.subject
  ...
  test "failed with invalid_export reason" do
    ...
    assert_match "isn't a #{JetKB::Brand::APP_NAME} account export", email.body.encoded
  end
end
